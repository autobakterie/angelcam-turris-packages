...
install:
	...
	ln -sf /tmp/reforis-angelcam-connector/reforis_static/reforis_angelcam_connector /tmp/reforis/reforis_static/
...
lint-web: venv
	$(VENV_BIN)/$(PYTHON) -m pylint --rcfile=pylintrc reforis_angelcam_connector
	$(VENV_BIN)/$(PYTHON) -m pycodestyle --config=pycodestyle reforis_angelcam_connector
...
create-messages:
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_angelcam_connector/translations/messages.pot .
init-langs: create-messages
	for lang in $(LANGS); do \
		$(VENV_BIN)/pybabel init \
		-i reforis_angelcam_connector/translations/messages.pot \
		-d reforis_angelcam_connector/translations/ -l $$lang \
	; done
update-messages:
	$(VENV_BIN)/pybabel update -i ./reforis_angelcam_connector/translations/messages.pot -d ./reforis_angelcam_connector/translations
compile-messages:
	$(VENV_BIN)/pybabel compile -f -d ./reforis_angelcam_connector/translations

clean:
	find . -name '*.pyc' -exec rm -f {} +
	rm -rf $(VENV_NAME) *.eggs *.egg-info dist build .cache
	rm -rf dist build *.egg-info
	rm -rf $(JS_DIR)/node_modules/ reforis_static/reforis_angelcam_connector/js/app.min.js
	$(PYTHON) -m pip uninstall -y reforis_angelcam_connector
